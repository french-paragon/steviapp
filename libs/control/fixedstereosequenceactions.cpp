...
void exportStereoImagesPlusColorPointCloud(FixedStereoPlusColorSequence* sequence, QVector<int> rows) {
	...
	if (rectifier == nullptr) {
		qDebug() << functionName << "Cannot configure rectifier";
		return;
	}
	...
	auto projectPoint = [] (StereoVision::Geometry::AffineTransform worldToCam,
			Eigen::Vector3f camPoint,
			std::optional<Eigen::Vector3f> const& r_dist,
			std::optional<Eigen::Vector2f> const& t_dist) -> Eigen::Vector2f {

		Eigen::Vector3f pt = worldToCam*camPoint;
		if (pt[2] < 0.0) {
			return {NAN, NAN};
		}

		Eigen::Vector2f proj = StereoVision::Geometry::projectPoints(pt);;
	...
	if (!ok) {
		qDebug() << functionName << "could not compute the rectifier";
		return;
	}
	...
	StereoVision::Geometry::AffineTransform cw_2_rgb = getWorldToImageTransform(rgbImg).value().toAffineTransform();
	...
	for (int row : rows) {
		...
		for (int i = 0; i < shape[0]; i++) {
			for (int j = 0; j < shape[1]; j++) {
				...
				Eigen::Vector2f rgbCamHomCoord = projectPoint(cw_2_rgb, worldCoord, r_dist, t_dist);

				Eigen::Vector2f rgbPixCoord = rgbFlen*rgbCamHomCoord + rgbPP;

				int col_i = std::round(rgbPixCoord.y());
				int col_j = std::round(rgbPixCoord.x());

				float r = arrayImRgb.valueOrAlt({col_i,col_j,0}, std::nanf(""));
				float g = arrayImRgb.valueOrAlt({col_i,col_j,1}, std::nanf(""));
				float b = arrayImRgb.valueOrAlt({col_i,col_j,2}, std::nanf(""));
